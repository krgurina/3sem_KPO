...
namespace LA
{
	...
	bool Analyse(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		...
		case 'd':
		{
			FST::FST* create = new FST::FST(CREATE(token));
			if (FST::execute(*create))
			{
				Add(lexTable, { LEX_CREATE, str_number, LT_TI_NULLXDX });
	...
	bool AnalyseIdentificator(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable, TypeOfVar& flag_type_variable)
	{
		...
		if (FST::execute(*id))
		{
			...
			//Переменная
			if (!Checked_id && (lexTable.GetEntry(lexTable.size - 1).lexema == 't' && lexTable.GetEntry(lexTable.size - 2).lexema == LEX_CREATE && flag_type_variable.LT_posititon == lexTable.size - 1))
			{
