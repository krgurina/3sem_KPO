...
namespace LT
{
	LT::LexTable::LexTable()
	{
		...
		this->size = 0;// текущий размер таблицы
		this->table = new Entry[LT_MAXSIZE];
	}
	
	void Add(LexTable& lextable, Entry entry)
	{
		if (lextable.size < lextable.maxsize) // проверка на переполнне таблицы
			lextable.table[lextable.size++] = entry;
		else
			throw ERROR_THROW(120);
	}
	Entry LexTable::GetEntry(int n) // получение лексемы из таблицы
	{
		if (n < this->maxsize && n >= 0) // проверка на наличие элемента в табоице
			return this->table[n];
	}
	...
	void LexTable::PrintLexTable(const wchar_t* in)
	{
		...
		if (lex_stream->is_open())
		{
			...
			cout << "Таблица лексем построена" << endl;
